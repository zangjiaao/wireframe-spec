---
name: wireframe-spec
description: Use when users ask for 线框图、wireframe、页面布局草图、信息架构预览或HTML布局预览，并需要统一的低保真规范输出（先确认端类型与页面范围）。
---
...
4. 默认端策略：`先确认后生成`（必须先确认桌面端/移动端/双端与前后台范围，不再默认双端同时生成）。
5. 若用户需求中已明确端类型与页面范围，可直接执行，不重复追问。
6. 默认启用：`WF Inspector`（元素抓取器）说明文案与快捷键约定。

## Output Contract
1. 只输出可编辑 HTML/CSS/少量原生 JS（不输出图片）。
2. 固定目录：`wireframes/`。
3. 固定主文件命名（按确认范围选择输出，可只交付其中一个）：
...
## Workflow
1. 先确认生成范围：端类型（桌面/移动/双端）、页面范围（前台/后台/都要）、交付文件名。
2. 从需求提取页面清单、关键状态、关键规则。
3. 先搭四行壳，再填每个 Tab 的卡片结构。
4. 使用组件字典完成占位，不写高保真视觉。
5. 把业务语义和约束补到第4行注释。
6. 对齐命名一致性（Tab/标题同义）。
7. 注入或保留元素抓取器（含 `Alt+点击` 上级容器能力）与顶部说明文案。
8. 生成并保存到 `wireframes/`。
